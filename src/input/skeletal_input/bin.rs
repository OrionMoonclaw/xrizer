...
use std::env;
use xrizer::input::{
    skeletal_input::{SkeletalInputActionData, SkeletalInputActionStates},
    Profiles,
};
...
fn main() {
    ...
    let (notify_sender, notify_receiver) = ipc::channel::<bool>().unwrap();
    let (result_sender, result_receiver) = ipc::channel::<SkeletalInputActionStates>().unwrap();
    ...
    loop {
        if let Ok(is_left) = notify_receiver.recv() {
            xr.sync_actions();
            let path = xr
                .instance
            ...
            if result_sender.send(xr.get_action_states(path)).is_err() {
                break;
            }
        } else {
            break;
